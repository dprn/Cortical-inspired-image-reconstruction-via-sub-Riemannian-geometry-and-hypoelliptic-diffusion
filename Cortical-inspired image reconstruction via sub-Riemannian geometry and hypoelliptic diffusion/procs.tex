...
% \textcolor{red}{the Laplacian,}  
% \textcolor{blue}{the equation $\partial_t \psi = \Delta\psi$ or $\partial_t \psi = \Delta^n \psi$,}  
...
% \textcolor{red}{the local properties} 
% \textcolor{blue}{the location of level lines} 
of the image.
}
% \textcolor{green}{CITATION from \cite{Tschum}: \quad 
% Typically, the principal smoothing directions are chosen to be parallel to the image contours, resulting in an anisotropic regularization that does not destroy edges. 
% As a requirement, deﬁning a correct smoothing behavior is one of the ﬁrst aim of a good regularization algorithm, the second being the precision of the smoothing process itself : it must respect the deﬁned smoothing geometry as much as possible. 
% }
...
% \textcolor{blue}{
% In the last two decades, PDE-based methods have led to a new field in image processing and computer vision,
% including inpainting, smoothing and image compression problems, see e.g., \cite{GWWBBS,Tschum,Weickert}.
% The central role in these methods belongs to evolution PDEs in the form $\partial_t \psi = L\psi$,
% where $\psi(t,x,y)$ gives the desired interpolating function, $(x,y)$ is a point of the image and $t \ge 0$
% is an evolution parameter (time). For inpainting, the evolution PDE is equipped with an initial and boundary
% conditions which represents the initial data. Here $L$ is a linear or nonlinear operator which defines
% the type of diffusion acting on the image.
% For instance, one can consider $L=\Delta$, $L=\Delta^2$, $L=\Delta^3$,
% %$L \psi = {\rm div} (g(|\nabla \psi|^2) \nabla \psi)$ with a special function $g$,
% where $\Delta = \partial_x^2+\partial_y^2$.
% }

% \textcolor{blue}{
% All the above examples are isotropic diffusions. This means that the effect of the diffusion in any direction is the same. For better quality of inpainting and smoothing, the idea is to reduce diffusion across level lines of the image, while still permitting it along them. Such diffusions are called anisotropic:
% the effect of anisotropic diffusion is more strong in directions with large gradients of the image,
% Several examples of anisotropic (edge-enhancing) diffusions and their applications
% are presented in \cite{GWWBBS,Tschum}.
% }

% \textcolor{blue}{
% In this paper, we present another type of anisotropic diffusion called {\it hypoelliptic}.
% It is bio-mimetic in the sense that it replicates the way in which the human primary visual cortex V1 processes the signals arriving from the eyes.
% This approach is inspired by a paradigm that dates back to the works of Hubel and Wiesel, Nobel prize winners in 1981. In contrast to anisotropic diffusions mentioned above, hypoelliptic diffusion acts not on the $(x,y)$-plane, but
% on a 3-dimensional space, the projective tangent bundle of the plane.
% This reflects the concept that neurons of V1 are sensitive to visual stimuli at a given point of the retina and for a given direction.
% }
