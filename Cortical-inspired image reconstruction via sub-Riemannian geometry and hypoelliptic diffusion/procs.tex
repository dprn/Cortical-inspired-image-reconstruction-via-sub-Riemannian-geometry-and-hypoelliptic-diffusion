...
In order to objectively evaluate the performance of the proposed algorithm, we consider, we compute the following quantities for the quality of a reconstruction $f$ of a corrupted version of an image $g$:
...
\end{equation}
Here, we used the \emph{average absolute error (AEE)} and the \emph{mean squared error (MSE)}, as in \cite{GWWBBS}, which are given by
\begin{equation}
